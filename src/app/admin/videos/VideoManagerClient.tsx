    ...
    <div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-border pb-4 gap-4">
        ...
        <div className="w-full md:w-64">
           <input
             ...
             className="w-full px-4 py-2 bg-secondary border border-border rounded text-sm outline-none focus:border-primary text-foreground"
           />
        </div>
      </div>
      
      <div className="bg-secondary border border-border rounded-xl p-6 backdrop-blur-md mb-8 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
         ...
         <form onSubmit={handleUpload} className="space-y-4">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div>
               ...
               <input name="title" required className="w-full bg-input/40 border border-border focus:border-primary outline-none transition-all rounded-lg py-2 px-4 mt-1 text-foreground" />
             </div>
             <div>
               <label className="text-xs font-bold text-muted-foreground ml-1">EXTERNAL HLS URL (OR UPLOAD)</label>
               <input name="externalUrl" type="url" placeholder="https://..." className="w-full bg-input/40 border border-border focus:border-primary outline-none transition-all rounded-lg py-2 px-4 mt-1 text-sm text-foreground" />
             </div>
             <div className="md:col-span-2">
               <label className="text-xs font-bold text-muted-foreground ml-1">DESCRIPTION</label>
               <textarea name="description" className="w-full bg-input/40 border border-border focus:border-primary outline-none rounded-lg py-2 px-4 mt-1 h-20 text-foreground"></textarea>
             </div>
             <div className="md:col-span-2">
               ...
               <input name="file" type="file" accept="video/*" className="w-full bg-input/40 border border-border focus:border-primary outline-none rounded-lg py-2 px-4 mt-1 text-sm text-muted-foreground file:bg-primary file:text-black file:border-0 file:cursor-pointer file:rounded file:px-4 file:py-1 file:mr-4 file:font-semibold" />
             </div>
           </div>
         ...
         <table className="w-full text-left">
           ...
           <tbody>
             {filteredVideos.map(video => (
               <tr key={video.id} className="border-b border-border hover:bg-secondary transition-colors">
                 <td className="p-4 text-sm font-semibold max-w-[200px] truncate text-foreground">{video.title}</td>
                 ...
                 <td className="p-4 text-center">
                   <div className="flex items-center justify-center gap-2">
                     <button onClick={() => setEditingVideo(video)} className="p-2 text-muted-foreground hover:text-foreground transition-colors" title={t('admin.video.edit')}>
      ...
      {editingVideo && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center px-4">
          <div className="bg-background border border-border rounded-xl p-8 max-w-lg w-full relative shadow-[0_0_50px_rgba(255,0,170,0.2)]">
             <button onClick={() => setEditingVideo(null)} className="absolute top-4 right-4 text-muted-foreground hover:text-foreground"><X className="w-5 h-5"/></button>
             <h2 className="text-2xl font-bold font-orbitron text-foreground mb-6">{t('admin.video.edit')}: {editingVideo.title}</h2>
             <form onSubmit={handleUpdate} className="space-y-4">
               ...
               <div>
                 ...
                 <input name="title" defaultValue={editingVideo.title} required className="w-full bg-input/40 border border-border focus:border-accent outline-none rounded-lg py-2 px-4 mt-1 text-foreground" />
               </div>
               <div>
                 <label className="text-xs font-bold text-muted-foreground">EXTERNAL URL</label>
                 <input name="externalUrl" type="url" defaultValue={editingVideo.url} required className="w-full bg-input/40 border border-border focus:border-accent outline-none rounded-lg py-2 px-4 mt-1 text-sm text-foreground" />
               </div>
               <div>
                 <label className="text-xs font-bold text-muted-foreground">DESCRIPTION</label>
                 <textarea name="description" defaultValue={editingVideo.description || ""} className="w-full bg-input/40 border border-border focus:border-accent outline-none rounded-lg py-2 px-4 mt-1 h-20 text-foreground"></textarea>
               </div>
               
               <button type="submit" disabled={loading} className="w-full mt-4 bg-accent text-foreground font-bold py-2 rounded tracking-widest shadow-[0_0_15px_rgba(255,0,170,0.4)]">
