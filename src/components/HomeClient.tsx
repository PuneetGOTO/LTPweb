...
export default function HomeClient({ topCompanions }: { topCompanions: any[] }) {
  ...
  return (
    <main className="flex-1 flex flex-col items-center bg-black min-h-screen relative overflow-x-hidden">
      <Navbar overlay={true} />
      ...
      <section className="relative w-full min-h-[90vh] flex flex-col items-center justify-center pt-20">
        <div className="z-10 flex flex-col items-center text-center px-4 max-w-5xl">
          ...
          <div className="absolute font-black font-orbitron text-[38vw] tracking-tighter text-white/5 blur-[8px] pointer-events-none select-none top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 drop-shadow-[0_0_30px_rgba(255,255,255,0.05)]" style={{ lineHeight: 0.8 }}>
             LTP
          </div>
          ...
          <motion.h1 
            ...
            className="text-5xl md:text-8xl font-black font-orbitron tracking-tighter mb-4 text-transparent bg-clip-text bg-gradient-to-br from-white via-primary to-accent drop-shadow-2xl relative z-10"
          >
            {t('hero.title')}
          </motion.h1>
          ...
          >
            ...
            <Link href="/video-hub" className="px-6 md:px-8 py-4 bg-white/5 hover:bg-white/10 text-white border border-white/10 font-bold text-lg rounded-lg transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2 backdrop-blur-md w-full sm:w-auto">
      ...
      <section className="w-full max-w-7xl mx-auto py-24 px-4 md:px-8 relative z-20">
        <h2 className="text-3xl md:text-4xl font-black font-orbitron text-white decoration-accent drop-shadow-[0_0_15px_rgba(255,0,170,0.5)] mb-2 md:inline-block border-b-4 border-accent pb-2 text-center md:text-left">
        ...
        {topCompanions.length === 0 ? (
           <div className="p-12 text-center border border-white/10 bg-white/5 rounded-xl backdrop-blur-md">
              <p className="text-muted-foreground">{t('home.empty')}</p>
           </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
             {topCompanions.map(companion => (
                <Link href={`/companion/${companion.id}`} key={companion.id} className="group flex flex-col bg-black/60 border border-white/10 rounded-2xl overflow-hidden hover:border-accent/60 transition-all hover:-translate-y-2 hover:shadow-[0_10px_40px_rgba(255,0,170,0.2)]">
                  <div className="h-48 bg-white/5 relative flex items-center justify-center">
                    {companion.profile?.avatarUrl ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-tr from-slate-900 to-black flex flex-col items-center justify-center">
                        <Users className="w-12 h-12 text-accent/50 mb-2" />
                      </div>
                    )}
                    <div className="absolute top-4 right-4 bg-black/80 backdrop-blur border border-white/10 rounded-full px-3 py-1 flex items-center gap-1">
                      <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
                      <span className="text-xs font-bold">{companion.profile?.rating?.toFixed(1) || "5.0"}</span>
                    </div>
                  </div>
                  <div className="p-6">
                    <h3 className="text-2xl font-bold font-orbitron tracking-wider text-white mb-1 group-hover:text-accent transition-colors">{companion.username}</h3>
                    ...
                    <div className="flex gap-2 mb-4 flex-wrap">
                      {(companion.profile?.games || "Valorant").split(',').map((g: string) => (
                        <span key={g} className="text-xs px-2 py-1 rounded bg-white/10 text-muted-foreground border border-white/5">{g.trim()}</span>
                      ))}
                    </div>
                    
                    <div className="w-full text-center py-2 bg-white/5 text-white/70 font-bold rounded group-hover:bg-accent group-hover:text-white transition-all text-sm tracking-widest mt-auto">
                      {t('companion.book')}
                    </div>
                  </div>
                </Link>
